...
data indata;
 ...
 if _n_ = &gridsize. then do;
	...
	/* loopa tills alla möjliga rullar är borta */
	do until (allRemoved or q>10000);
		prevAntal = totantal;
		q+1;
		...
		%printgrid(grid=adot);
		/* kopiera adot till grid så att de borttagna rullarna tas bort och kör om */
		 do y = 1 to &gridsize.;
		 	do x = 1 to &gridsize.;
			  grid{x,y} = adot{x,y};
			end;
		 ...
		 /* om inga nya rullar tagits bort så är vi klara */
		 if totantal = prevAntal then allRemoved = 1;
